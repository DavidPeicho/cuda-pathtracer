...
static int
stbtt_InitFont_internal(stbtt_fontinfo* info, unsigned char* data,
                        int fontstart)
{
  stbtt_uint32 cmap, t;
...
static void
stbtt_setvertex(stbtt_vertex* v, stbtt_uint8 type, stbtt_int32 x, stbtt_int32 y,
...
static int
stbtt__close_shape(stbtt_vertex* vertices, int num_vertices, int was_off,
...
static int
stbtt__GetGlyphShapeTT(const stbtt_fontinfo* info, int glyph_index,
...
static void
stbtt__csctx_v(stbtt__csctx* c, stbtt_uint8 type, stbtt_int32 x, stbtt_int32 y,
...
static void
stbtt__csctx_rccurve_to(stbtt__csctx* ctx, float dx1, float dy1, float dx2,
...
static int
stbtt__run_charstring(const stbtt_fontinfo* info, int glyph_index,
                      stbtt__csctx* c)
{
...
static int
stbtt__GetGlyphShapeT2(const stbtt_fontinfo* info, int glyph_index,
                       stbtt_vertex** pvertices)
{
...
static int
stbtt__GetGlyphInfoT2(const stbtt_fontinfo* info, int glyph_index, int* x0,
                      int* y0, int* x1, int* y1)
{
...
STBTT_DEF int
stbtt_GetGlyphShape(const stbtt_fontinfo* info, int glyph_index,
                    stbtt_vertex** pvertices)
{
  if (!info->cff.size)
...
stbtt__new_active(stbtt__hheap* hh, stbtt__edge* e, int off_x,
...
static void
stbtt__fill_active_edges(unsigned char* scanline, int len,
...
static void
stbtt__rasterize_sorted_edges(stbtt__bitmap* result, stbtt__edge* e, int n,
...
static void
stbtt__handle_clipped_edge(float* scanline, int x, stbtt__active_edge* e,
...
static void
stbtt__fill_active_edges_new(float* scanline, float* scanline_fill, int len,
...
static void
stbtt__rasterize(stbtt__bitmap* result, stbtt__point* pts, int* wcount,
...
static int
stbtt__tesselate_curve(stbtt__point* points, int* num_points, float x0,
...
static void
stbtt__tesselate_cubic(stbtt__point* points, int* num_points, float x0,
                       float y0, float x1, float y1, float x2, float y2,
...
static stbtt__point*
stbtt_FlattenCurves(stbtt_vertex* vertices, int num_verts,
...
STBTT_DEF void
stbtt_GetBakedQuad(stbtt_bakedchar* chardata, int pw, int ph, int char_index,
...
static void
stbrp_init_target(stbrp_context* con, int pw, int ph, stbrp_node* nodes,
                  int num_nodes)
{
  con->width = pw;
  con->height = ph;
...
STBTT_DEF int
stbtt_PackBegin(stbtt_pack_context* spc, unsigned char* pixels, int pw, int ph,
...
static void
stbtt__h_prefilter(unsigned char* pixels, int w, int h, int stride_in_bytes,
...
static void
stbtt__v_prefilter(unsigned char* pixels, int w, int h, int stride_in_bytes,
                   unsigned int kernel_width)
{
  unsigned char buffer[STBTT_MAX_OVERSAMPLE];
...
STBTT_DEF int
stbtt_PackFontRanges(stbtt_pack_context* spc, unsigned char* fontdata,
...
STBTT_DEF int
stbtt_PackFontRange(stbtt_pack_context* spc, unsigned char* fontdata,
...
STBTT_DEF void
stbtt_GetPackedQuad(stbtt_packedchar* chardata, int pw, int ph, int char_index,
                    float* xpos, float* ypos, stbtt_aligned_quad* q,
                    int align_to_integer)
{
  float ipw = 1.0f / pw, iph = 1.0f / ph;
...
static int
stbtt_CompareUTF8toUTF16_bigendian_internal(char* s1, int len1, char* s2,
                                            int len2)
{
...
static int
stbtt__matchpair(stbtt_uint8* fc, stbtt_uint32 nm, stbtt_uint8* name,
...
static int
stbtt__matches(stbtt_uint8* fc, stbtt_uint32 offset, stbtt_uint8* name,
               stbtt_int32 flags)
{
...
static int
stbtt_FindMatchingFont_internal(unsigned char* font_collection, char* name_utf8,
                                stbtt_int32 flags)
{
  stbtt_int32 i;
  for (i = 0;; ++i) {
...
STBTT_DEF int
stbtt_BakeFontBitmap(const unsigned char* data, int offset, float pixel_height,
