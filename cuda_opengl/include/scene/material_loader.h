...
{
public:
  static inline MaterialLoader* instance()
  {
    if (inst == nullptr)
...
public:
  ...
  /// <param name="mtl_folder">Base folder containing the assets (MTL,
  /// etc..)</param>
  /// <returns></returns>
  ...
  /// </summary>
  /// <param name="">Contains the loaded material, ready to be upload to the
  /// GPU.</param>
  ...
  inline const std::vector<scene::Texture>& getTextures() const
  {
    return _textures;
  }

private:
  int getTextureId(std::string tex_rgb);

  int getTextureId(std::string tex_rgb, float3 default_rgb);

  int getTextureId(std::string tex_rgb, std::string tex_a, float3 default_rgb,
                   float default_a);

  int registerOrGet(std::string tex_rgb);
