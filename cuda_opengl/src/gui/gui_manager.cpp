...
namespace {
bool
vectorGetter(void* data, int n, const char** out_text)
{
                 ...
                 const std::vector<std::string>& cubemaps)
{
  ...
  ImGui::ListBox("Scenes", &scene_id, vectorGetter, (void*)&items,
                 (int)items.size(), MAX_DISPLAY_ELT);
  ImGui::ListBox("Cubemaps", &cubemap_id, vectorGetter, (void*)&cubemaps,
                 (int)cubemaps.size(), MAX_DISPLAY_ELT);
  ImGui::End();
...
GUIManager::camera(scene::Camera& cam, float h_offset)
{
  ...
  ImGui::Separator();
  ImGui::SliderFloat("Aperture size", &cam.aperture, MIN_APERTURE,
                     MAX_APERTURE);
