...
/*
 ...
 integration process, you can read examples/opengl2_example/ because
 ...
 within string itself.
     This helps solving the simpler collision cases. Use "##" to pass a
 complement to the ID that won't be visible to the end-user:
 ...
 NoTitleBar|NoResize|NoMove|NoScrollbar|NoSavedSettings|NoInputs flag,
    zero background alpha, then retrieve the ImDrawList* via GetWindowDrawList()
 and draw to it in any way you like.
    You can also perfectly create a standalone ImDrawList instance _but_ you
...
#pragma clang diagnostic ignored                                               \
  "-Wfloat-equal" // warning : comparing floating point with == or != is unsafe
                  ...
                  // 0.0f) is ok.
#pragma clang diagnostic ignored "-Wformat-nonliteral" // warning : format
                                                       // string is not a string
                                                       // literal
                                                       // // passing non-literal
                                                       // to vsnformat(). yes,
                                                       // user passing incorrect
                                                       // format strings can
                                                       // crash the code.
#pragma clang diagnostic ignored                                               \
  "-Wexit-time-destructors" // warning : declaration requires an exit-time
                            ...
                            // that has been disabled before exit it might cause
...
// If you are hot-reloading this code in a DLL you will lose the static/global
// variables. Create your own context+font atlas instead of relying on those
// default (see FAQ entry "How can I preserve my ImGui context across reloading
// a DLL?").
...
// Tips: if you need to change your scale multiple times, prefer calling this on
// a freshly initialized ImGuiStyle structure rather than scaling multiple times
// (because floating point multiplications are lossy).
void
...
// WM_CHAR message
void
ImGuiIO::AddInputCharacter(ImWchar c)
{
...
bool
ImTriangleContainsPoint(const ImVec2& a, const ImVec2& b, const ImVec2& c,
                        const ImVec2& p)
{
...
void
ImTriangleBarycentricCoords(const ImVec2& a, const ImVec2& b, const ImVec2& c,
...
ImVec2
ImTriangleClosestPoint(const ImVec2& a, const ImVec2& b, const ImVec2& c,
                       const ImVec2& p)
{
...
int
ImStrnicmp(const char* str1, const char* str2, int count)
{
  int d = 0;
...
void
ImStrncpy(char* dst, const char* src, int count)
{
  if (count < 1)
    return;
...
static const char*
ImAtoi(const char* src, int* output)
{
  int negative = 0;
  if (*src == '-') {
    negative = 1;
...
int
ImFormatString(char* buf, int buf_size, const char* fmt, ...)
{
  va_list args;
  va_start(args, fmt);
...
int
ImFormatStringV(char* buf, int buf_size, const char* fmt, va_list args)
{
  int w = vsnprintf(buf, buf_size, fmt, args);
  if (buf == NULL)
    return w;
...
ImU32
ImHash(const void* data, int data_size, ImU32 seed)
{
...
void
ImGui::ColorConvertRGBtoHSV(float r, float g, float b, float& out_h,
...
void
ImGui::ColorConvertHSVtoRGB(float h, float s, float v, float& out_r,
...
// ImGui::MemFree()
void*
ImFileLoadToMemory(const char* filename, const char* file_open_mode,
...
void
ImGuiTextFilter::TextRange::split(char separator, ImVector<TextRange>& out)
{
  out.resize(0);
  const char* wb = b;
  const char* we = wb;
...
void
ImGuiTextBuffer::appendv(const char* fmt, va_list args)
{
  va_list args_copy;
...
void
ImGuiTextBuffer::append(const char* fmt, ...)
{
  va_list args;
  va_start(args, fmt);
  appendv(fmt, args);
...
void
ImGuiSimpleColumns::Update(int count, float spacing, bool clear)
{
...
float
ImGuiSimpleColumns::CalcExtraSpace(float avail_w)
{
...
static void
SetCursorPosYAndSetupDummyPrevLine(float pos_y, float line_height)
{
...
void
ImGuiListClipper::Begin(int count, float items_height)
{
...
ImGuiID
ImGuiWindow::GetIDNoKeepAlive(const char* str, const char* str_end)
{
  ImGuiID seed = IDStack.back();
...
static inline bool
IsWindowContentHoverable(ImGuiWindow* window, ImGuiHoveredFlags flags)
{
...
void
ImGui::ItemSize(const ImVec2& size, float text_offset_y)
{
  ImGuiContext& g = *GImGui;
  ...
  window->DC.CursorPos = ImVec2(
    (float)(int)(window->Pos.x + window->DC.IndentX +
...
void
ImGui::ItemSize(const ImRect& bb, float text_offset_y)
{
...
bool
ImGui::ItemAdd(const ImRect& bb, ImGuiID id)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
bool
ImGui::ItemHoverable(const ImRect& bb, ImGuiID id)
{
  ImGuiContext& g = *GImGui;
...
bool
ImGui::IsClippedEx(const ImRect& bb, ImGuiID id, bool clip_even_when_logged)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
bool
ImGui::FocusableItemRegister(ImGuiWindow* window, ImGuiID id, bool tab_stop)
{
  ImGuiContext& g = *GImGui;
...
void
ImGui::FocusableItemUnregister(ImGuiWindow* window)
{
  window->FocusIdxAllCounter--;
  window->FocusIdxTabCounter--;
}
...
ImVec2
ImGui::CalcItemSize(ImVec2 size, float default_x, float default_y)
{
  ImGuiContext& g = *GImGui;
  ImVec2 content_max;
...
ImGuiContext*
ImGui::CreateContext(void* (*malloc_fn)(size_t), void (*free_fn)(void*))
{
  if (!malloc_fn)
    malloc_fn = malloc;
...
ImGui::NewFrame()
{
  ...
  IM_ASSERT(g.Style.Alpha >= 0.0f &&
            g.Style.Alpha <= 1.0f); // Invalid style setting. Alpha cannot be
                                    // negative (allows us to avoid a few clamps
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.KeysDown); i++)
    g.IO.KeysDownDuration[i] =
      g.IO.KeysDown[i]
        ? (g.IO.KeysDownDuration[i] < 0.0f ? 0.0f : g.IO.KeysDownDuration[i] +
                                                      g.IO.DeltaTime)
        : -1.0f;
  ...
  for (int i = 0; i < IM_ARRAYSIZE(g.IO.MouseDown); i++) {
    ...
    g.IO.MouseDownDuration[i] =
      g.IO.MouseDown[i]
        ? (g.IO.MouseDownDuration[i] < 0.0f ? 0.0f : g.IO.MouseDownDuration[i] +
                                                       g.IO.DeltaTime)
        : -1.0f;
  ...
  // hovering.
  // FIXME: For patterns of drag and drop between "application" and "imgui" we
  // may need to rework/remove this test (first committed 311c0ca9 on 2015/02)
...
static ImGuiIniData*
FindWindowSettings(const char* name)
{
  ImGuiContext& g = *GImGui;
...
static ImGuiIniData*
AddWindowSettings(const char* name)
{
...
static void
AddWindowToSortedBuffer(ImVector<ImGuiWindow*>& out_sorted_windows,
                        ImGuiWindow* window)
{
...
static void
AddDrawListToRenderList(ImVector<ImDrawList*>& out_render_list,
                        ImDrawList* draw_list)
{
...
static void
AddWindowToRenderList(ImVector<ImDrawList*>& out_render_list,
                      ImGuiWindow* window)
{
...
static void
AddWindowToRenderListSelectLayer(ImGuiWindow* window)
{
...
void
ImGui::PushClipRect(const ImVec2& clip_rect_min, const ImVec2& clip_rect_max,
...
static void
LogRenderedText(const ImVec2* ref_pos, const char* text,
...
void
ImGui::RenderText(ImVec2 pos, const char* text, const char* text_end,
...
void
ImGui::RenderTextWrapped(ImVec2 pos, const char* text, const char* text_end,
                         float wrap_width)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
void
ImGui::RenderTextClipped(const ImVec2& pos_min, const ImVec2& pos_max,
...
void
ImGui::RenderFrame(ImVec2 p_min, ImVec2 p_max, ImU32 fill_col, bool border,
                   float rounding)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
void
ImGui::RenderTriangle(ImVec2 p_min, ImGuiDir dir, float scale)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
void
ImGui::RenderCheckMark(ImVec2 pos, ImU32 col, float sz)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
ImVec2
ImGui::CalcTextSize(const char* text, const char* text_end,
...
void
ImGui::CalcListClipping(int items_count, float items_height,
...
static bool
IsKeyPressedMap(ImGuiKey key, bool repeat)
{
...
int
ImGui::CalcTypematicPressedRepeatAmount(float t, float t_prev,
...
int
ImGui::GetKeyPressedAmount(int key_index, float repeat_delay, float repeat_rate)
{
  ImGuiContext& g = *GImGui;
  if (key_index < 0)
    return false;
...
bool
ImGui::IsKeyPressed(int user_key_index, bool repeat)
{
  ImGuiContext& g = *GImGui;
  if (user_key_index < 0)
    return false;
...
bool
ImGui::IsMouseClicked(int button, bool repeat)
{
  ImGuiContext& g = *GImGui;
  IM_ASSERT(button >= 0 && button < IM_ARRAYSIZE(g.IO.MouseDown));
...
bool
ImGui::IsMouseDragging(int button, float lock_threshold)
{
  ImGuiContext& g = *GImGui;
  IM_ASSERT(button >= 0 && button < IM_ARRAYSIZE(g.IO.MouseDown));
...
ImVec2
ImGui::GetMouseDragDelta(int button, float lock_threshold)
{
  ImGuiContext& g = *GImGui;
  IM_ASSERT(button >= 0 && button < IM_ARRAYSIZE(g.IO.MouseDown));
  if (lock_threshold < 0.0f)
...
void
ImGui::ResetMouseDragDelta(int button)
{
  ImGuiContext& g = *GImGui;
  IM_ASSERT(button >= 0 && button < IM_ARRAYSIZE(g.IO.MouseDown));
...
void
ImGui::CaptureKeyboardFromApp(bool capture)
{
  GImGui->WantCaptureKeyboardNextFrame = capture ? 1 : 0;
}

void
ImGui::CaptureMouseFromApp(bool capture)
{
...
bool
ImGui::IsItemClicked(int mouse_button)
{
...
ImVec2
ImGui::CalcItemRectClosestPoint(const ImVec2& pos, bool on_edge, float outward)
{
  ImGuiWindow* window = GetCurrentWindowRead();
...
void
ImGui::BeginTooltipEx(ImGuiWindowFlags extra_flags,
...
void
ImGui::OpenPopupEx(ImGuiID id, bool reopen_existing)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  ...
  ImGuiPopupRef popup_ref =
    ImGuiPopupRef(id, window, window->GetID("##menus"),
                  g.IO.MousePos); // Tagged as new ref because constructor sets
                                  // Window to NULL (we are passing the
  ...
  if (g.OpenPopupStack.Size < current_stack_size + 1)
    ...
    // parent is itself a popup it would get closed by CloseInactivePopups().
...
static void
CloseInactivePopups(ImGuiWindow* ref_window)
{
  ImGuiContext& g = *GImGui;
...
static void
ClosePopupToLevel(int remaining)
{
  ImGuiContext& g = *GImGui;
  if (remaining > 0)
...
bool
ImGui::BeginPopupEx(ImGuiID id, ImGuiWindowFlags extra_flags)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
  if (!IsPopupOpen(id)) {
...
bool
ImGui::BeginPopup(const char* str_id)
{
  ImGuiContext& g = *GImGui;
...
bool
ImGui::BeginPopupModal(const char* name, bool* p_open,
...
bool
ImGui::OpenPopupOnItemClick(const char* str_id, int mouse_button)
{
...
bool
ImGui::BeginPopupContextItem(const char* str_id, int mouse_button)
{
  ImGuiWindow* window = GImGui->CurrentWindow;
  ImGuiID id = str_id ? window->GetID(str_id)
                      : window->DC.LastItemId; // If user hasn't passed an ID,
                                               ...
                                               // won't conflict!
  IM_ASSERT(id != 0); // However, you cannot pass a NULL str_id if the last item
                      // has no identifier (e.g. a Text() item)
  if (IsMouseClicked(mouse_button) &&
      IsItemHovered(ImGuiHoveredFlags_AllowWhenBlockedByPopup))
    OpenPopupEx(id, true);
  return BeginPopupEx(id, ImGuiWindowFlags_ShowBorders |
                            ImGuiWindowFlags_AlwaysAutoResize);
}

bool
ImGui::BeginPopupContextWindow(const char* str_id, int mouse_button,
...
bool
ImGui::BeginPopupContextVoid(const char* str_id, int mouse_button)
{
  if (!str_id)
    str_id = "void_context";
  ImGuiID id = GImGui->CurrentWindow->GetID(str_id);
...
static bool
BeginChildEx(const char* name, ImGuiID id, const ImVec2& size_arg, bool border,
             ImGuiWindowFlags extra_flags)
{
...
bool
ImGui::BeginChild(const char* str_id, const ImVec2& size_arg, bool border,
                  ImGuiWindowFlags extra_flags)
{
  ImGuiWindow* window = GetCurrentWindow();
...
bool
ImGui::BeginChild(ImGuiID id, const ImVec2& size_arg, bool border,
                  ImGuiWindowFlags extra_flags)
{
...
// widget frame.
bool
ImGui::BeginChildFrame(ImGuiID id, const ImVec2& size,
                       ImGuiWindowFlags extra_flags)
{
  ImGuiContext& g = *GImGui;
...
static void
CheckStacksSize(ImGuiWindow* window, bool write)
{
...
static ImVec2
FindBestPopupWindowPos(const ImVec2& base_pos, const ImVec2& size,
                       int* last_dir, const ImRect& r_inner)
{
  ...
  r_outer.Expand(ImVec2(
    (size.x - r_outer.GetWidth() > safe_padding.x * 2) ? -safe_padding.x : 0.0f,
...
static ImGuiWindow*
CreateNewWindow(const char* name, ImVec2 size, ImGuiWindowFlags flags)
{
  ImGuiContext& g = *GImGui;
...
static ImVec2
CalcSizeFullWithConstraint(ImGuiWindow* window, ImVec2 new_size)
{
  ImGuiContext& g = *GImGui;
...
static ImVec2
CalcSizeAutoFit(ImGuiWindow* window)
{
  ImGuiContext& g = *GImGui;
  ...
  if ((flags & ImGuiWindowFlags_Tooltip) != 0) {
    ...
    size_auto_fit =
      ImClamp(window->SizeContents + window->WindowPadding, style.WindowMinSize,
    ...
    scroll.x = ImMin(
      ...
      ImMax(0.0f,
            window->SizeContents.x -
              (window->SizeFull.x -
               window->ScrollbarSizes.x))); // == GetScrollMaxX for that window
    scroll.y = ImMin(
      scroll.y,
      ImMax(0.0f,
            window->SizeContents.y -
              (window->SizeFull.y -
               window->ScrollbarSizes.y))); // == GetScrollMaxY for that window
  }
  return scroll;
}
...
ImGui::Begin(const char* name, bool* p_open, ImGuiWindowFlags flags)
{
  ...
  if (flags & ImGuiWindowFlags_Popup) {
    ...
    window_just_activated_by_user |=
      (window->PopupId != popup_ref.PopupId); // We recycle popups so treat
                                              // window as activated if popup id
                                              // changed
  ...
  window->RootNonPopupWindow =
    g.CurrentWindowStack[root_non_popup_idx]; // Used to display TitleBgActive
                                              // color and for selecting which
  ...
  if (first_begin_of_the_frame) {
    ...
    if (window_pos_with_pivot) {
      ...
      float horizontal_overlap =
        style.ItemSpacing.x; // We want some overlap to convey the relative
                             ...
                             // overlap it is hard-coded to style.ItemSpacing.x,
                             // may need to introduce another style value).
    ...
    if ((flags & ImGuiWindowFlags_Tooltip) != 0 && !window_pos_set_by_api) {
      ...
      if (window->AutoPosLastDirection == -1)
        window->PosFloat =
          ref_pos + ImVec2(2, 2); // If there's not enough room, for tooltip we
                                  // prefer avoiding the cursor at all cost even
                                  // if it means that part of the tooltip won't
                                  // be visible.
    }
    ...
    if (window->Collapsed) {
      ...
      if (flags & ImGuiWindowFlags_ShowBorders) {
        window->DrawList->AddRect(
          window->Pos + ImVec2(1, 1), window->Pos + window->Size + ImVec2(1, 1),
...
bool
ImGui::Begin(const char* name, bool* p_open, const ImVec2& size_on_first_use,
             float bg_alpha_override, ImGuiWindowFlags flags)
{
  // Old API feature: we could pass the initial window size as a parameter,
  // however this was very misleading because in most cases it would only affect
  // the window when it didn't have storage in the .ini file.
  ...
  // parameter. This is actually tricky to reproduce manually because:
  // (1) there are multiple variants of WindowBg (popup, tooltip, etc.) and (2)
  // you can't call PushStyleColor before Begin and PopStyleColor just after
  // Begin() because of how CheckStackSizes() behave.
  // The user-side solution is to do backup = GetStyleColorVec4(ImGuiCol_xxxBG),
  // PushStyleColor(ImGuiCol_xxxBg), Begin, PushStyleColor(ImGuiCol_xxxBg,
  // backup), [...], PopStyleColor(), End(); PopStyleColor() - which is super
  // awkward.
  // The alpha override was rarely used but for now we'll leave the Begin()
  // variant around for a bit. We may either lift the constraint on
...
void
ImGui::Scrollbar(ImGuiLayoutType direction)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = g.CurrentWindow;
...
static const ImGuiStyleVarInfo*
...
bool
ImGui::IsRootWindowOrAnyChildHovered(ImGuiHoveredFlags flags)
{
  IM_ASSERT((flags & ImGuiHoveredFlags_AllowWhenOverlapped) ==
            0); // Flags not supported by this function
  ImGuiContext& g = *GImGui;
...
static void
SetWindowScrollY(ImGuiWindow* window, float new_scroll_y)
{
  window->DC.CursorMaxPos.y +=
...
static void
SetWindowPos(ImGuiWindow* window, const ImVec2& pos, ImGuiCond cond)
{
...
static void
SetWindowSize(ImGuiWindow* window, const ImVec2& size, ImGuiCond cond)
{
  // Test condition (NB: bit 0 is always true) and clear flags for next time
...
ImGui::GetContentRegionMax()
{
  ...
  if (window->DC.ColumnsCount != 1)
    mx.x =
      GetColumnOffset(window->DC.ColumnsCurrent + 1) - window->WindowPadding.x;
  return mx;
}
...
void
ImGui::SetScrollFromPosY(float pos_y, float center_y_ratio)
{
  ...
  if (center_y_ratio <= 0.0f &&
      window->ScrollTarget.y <=
        window->WindowPadding.y) // Minor hack to make "scroll to top" take
                                 ...
                                 // scroll to (WindowPadding.y - ItemSpacing.y)
...
void
ImGui::SetScrollHere(float center_y_ratio)
{
  ImGuiWindow* window = GetCurrentWindow();
...
bool
ImGui::ButtonBehavior(const ImRect& bb, ImGuiID id, bool* out_hovered,
...
bool
ImGui::ButtonEx(const char* label, const ImVec2& size_arg,
                ImGuiButtonFlags flags)
{
  ...
  if ((flags & ImGuiButtonFlags_AlignTextBaseLine) &&
      style.FramePadding.y <
        window->DC.CurrentLineTextBaseOffset) // Try to vertically align buttons
  ...
  const ImU32 col = GetColorU32(
    (hovered && held) ? ImGuiCol_ButtonActive : hovered ? ImGuiCol_ButtonHovered
                                                        : ImGuiCol_Button);
...
bool
ImGui::InvisibleButton(const char* str_id, const ImVec2& size_arg)
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
...
bool
ImGui::CloseButton(ImGuiID id, const ImVec2& pos, float radius)
{
  ImGuiWindow* window = GetCurrentWindow();
...
void
ImGui::Image(ImTextureID user_texture_id, const ImVec2& size, const ImVec2& uv0,
...
void
ImGui::LogToFile(int max_depth, const char* filename)
{
  ImGuiContext& g = *GImGui;
  if (g.LogEnabled)
    return;
...
void
ImGui::LogToClipboard(int max_depth)
{
  ImGuiContext& g = *GImGui;
  if (g.LogEnabled)
    return;
...
bool
ImGui::TreeNodeBehaviorIsOpen(ImGuiID id, ImGuiTreeNodeFlags flags)
{
...
bool
ImGui::TreeNodeBehavior(ImGuiID id, ImGuiTreeNodeFlags flags, const char* label,
                        const char* label_end)
{
  ...
  const float frame_height = ImMax(
    ImMin(window->DC.CurrentLineHeight, g.FontSize + style.FramePadding.y * 2),
    label_size.y + padding.y * 2);
  ImRect bb =
    ImRect(window->DC.CursorPos, ImVec2(window->Pos.x + GetContentRegionMax().x,
  ...
  const ImRect interact_bb =
    display_frame ? bb : ImRect(bb.Min.x, bb.Min.y,
  ...
  // Render
  const ImU32 col = GetColorU32(
    (held && hovered) ? ImGuiCol_HeaderActive : hovered ? ImGuiCol_HeaderHovered
                                                        : ImGuiCol_Header);
...
bool
ImGui::CollapsingHeader(const char* label, bool* p_open,
                        ImGuiTreeNodeFlags flags)
{
  ...
  bool is_open =
    TreeNodeBehavior(id, flags | ImGuiTreeNodeFlags_CollapsingHeader |
                           ImGuiTreeNodeFlags_NoTreePushOnOpen |
  ...
  if (p_open) {
    ...
    if (CloseButton(
          ...
          ImVec2(ImMin(window->DC.LastItemRect.Max.x, window->ClipRect.Max.x) -
                 ...
                 window->DC.LastItemRect.Min.y + g.Style.FramePadding.y +
                   button_sz),
          button_sz))
      *p_open = false;
...
bool
ImGui::TreeNodeExV(const char* str_id, ImGuiTreeNodeFlags flags,
...
bool
ImGui::TreeNodeExV(const void* ptr_id, ImGuiTreeNodeFlags flags,
                   const char* fmt, va_list args)
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
...
bool
ImGui::TreeNodeEx(const char* str_id, ImGuiTreeNodeFlags flags, const char* fmt,
                  ...)
{
  va_list args;
  va_start(args, fmt);
...
bool
ImGui::TreeNodeEx(const void* ptr_id, ImGuiTreeNodeFlags flags, const char* fmt,
                  ...)
{
  va_list args;
  va_start(args, fmt);
...
void
ImGui::SetNextTreeNodeOpen(bool is_open, ImGuiCond cond)
{
  ImGuiContext& g = *GImGui;
...
static inline void
DataTypeFormatString(ImGuiDataType data_type, void* data_ptr,
...
bool
ImGui::InputScalarAsWidgetReplacement(const ImRect& aabb, const char* label,
                                      ImGuiDataType data_type, void* data_ptr,
...
int
ImGui::ParseFormatPrecision(const char* fmt, int default_precision)
{
...
float
ImGui::RoundScalar(float value, int decimal_precision)
{
...
static inline float
SliderBehaviorCalcRatioFromValue(float v, float v_min, float v_max, float power,
...
bool
ImGui::SliderBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_min,
                      ...
                      ImGuiSliderFlags flags)
{
  ...
  if (decimal_precision != 0)
    ...
    grab_sz = ImMin(
      ImMax(1.0f * (slider_sz /
            ...
            style.GrabMinSize),
      slider_sz); // Integer sliders, if possible have the grab size represent 1
                  // unit
  ...
  // Draw
  float grab_t =
    SliderBehaviorCalcRatioFromValue(*v, v_min, v_max, power, linear_zero_pos);
  if (!is_horizontal)
    grab_t = 1.0f - grab_t;
  ...
  if (is_horizontal)
    grab_bb =
      ImRect(ImVec2(grab_pos - grab_sz * 0.5f, frame_bb.Min.y + grab_padding),
             ImVec2(grab_pos + grab_sz * 0.5f, frame_bb.Max.y - grab_padding));
  else
    grab_bb =
      ImRect(ImVec2(frame_bb.Min.x + grab_padding, grab_pos - grab_sz * 0.5f),
...
bool
ImGui::VSliderFloat(const char* label, const ImVec2& size, float* v,
...
bool
ImGui::SliderInt(const char* label, int* v, int v_min, int v_max,
                 const char* display_format)
{
  ...
  float v_f = (float)*v;
  bool value_changed =
    SliderFloat(label, &v_f, (float)v_min, (float)v_max, display_format, 1.0f);
  *v = (int)v_f;
  return value_changed;
}

bool
ImGui::VSliderInt(const char* label, const ImVec2& size, int* v, int v_min,
...
bool
ImGui::SliderFloatN(const char* label, float* v, int components, float v_min,
...
bool
ImGui::SliderIntN(const char* label, int* v, int components, int v_min,
                  int v_max, const char* display_format)
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
...
bool
ImGui::SliderInt2(const char* label, int v[2], int v_min, int v_max,
                  const char* display_format)
{
...
bool
ImGui::SliderInt3(const char* label, int v[3], int v_min, int v_max,
                  const char* display_format)
{
...
bool
ImGui::SliderInt4(const char* label, int v[4], int v_min, int v_max,
                  const char* display_format)
{
...
bool
ImGui::DragBehavior(const ImRect& frame_bb, ImGuiID id, float* v, float v_speed,
                    float v_min, float v_max, int decimal_precision,
                    float power)
{
  ImGuiContext& g = *GImGui;
  const ImGuiStyle& style = g.Style;
...
bool
ImGui::DragFloat(const char* label, float* v, float v_speed, float v_min,
                 float v_max, const char* display_format, float power)
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
...
bool
ImGui::DragFloatN(const char* label, float* v, int components, float v_speed,
                  float v_min, float v_max, const char* display_format,
                  float power)
{
  ImGuiWindow* window = GetCurrentWindow();
...
bool
ImGui::DragFloat2(const char* label, float v[2], float v_speed, float v_min,
                  float v_max, const char* display_format, float power)
{
...
bool
ImGui::DragFloat3(const char* label, float v[3], float v_speed, float v_min,
                  float v_max, const char* display_format, float power)
{
...
bool
ImGui::DragFloat4(const char* label, float v[4], float v_speed, float v_min,
                  float v_max, const char* display_format, float power)
{
...
bool
ImGui::DragFloatRange2(const char* label, float* v_current_min,
...
bool
ImGui::DragInt(const char* label, int* v, float v_speed, int v_min, int v_max,
               const char* display_format)
{
  if (!display_format)
    display_format = "%.0f";
...
bool
ImGui::DragIntN(const char* label, int* v, int components, float v_speed,
...
bool
ImGui::DragInt2(const char* label, int v[2], float v_speed, int v_min,
                int v_max, const char* display_format)
{
...
bool
ImGui::DragInt3(const char* label, int v[3], float v_speed, int v_min,
                int v_max, const char* display_format)
{
...
bool
ImGui::DragInt4(const char* label, int v[4], float v_speed, int v_min,
                int v_max, const char* display_format)
{
...
bool
ImGui::DragIntRange2(const char* label, int* v_current_min, int* v_current_max,
...
void
ImGui::PlotLines(const char* label, float (*values_getter)(void* data, int idx),
...
bool
ImGui::CheckboxFlags(const char* label, unsigned int* flags,
...
static int
STB_TEXTEDIT_STRINGLEN(const STB_TEXTEDIT_STRING* obj)
{
  return obj->CurLenW;
}
static ImWchar
STB_TEXTEDIT_GETCHAR(const STB_TEXTEDIT_STRING* obj, int idx)
{
  return obj->Text[idx];
}
static float
STB_TEXTEDIT_GETWIDTH(STB_TEXTEDIT_STRING* obj, int line_start_idx,
                      int char_idx)
{
...
static void
STB_TEXTEDIT_LAYOUTROW(StbTexteditRow* r, STB_TEXTEDIT_STRING* obj,
                       int line_start_idx)
{
...
static int
is_word_boundary_from_right(STB_TEXTEDIT_STRING* obj, int idx)
{
  return idx > 0
           ? (is_separator(obj->Text[idx - 1]) && !is_separator(obj->Text[idx]))
           : 1;
}
static int
STB_TEXTEDIT_MOVEWORDLEFT_IMPL(STB_TEXTEDIT_STRING* obj, int idx)
{
  idx--;
...
static int
is_word_boundary_from_left(STB_TEXTEDIT_STRING* obj, int idx)
{
  return idx > 0
           ? (!is_separator(obj->Text[idx - 1]) && is_separator(obj->Text[idx]))
           : 1;
...
static int
STB_TEXTEDIT_MOVEWORDRIGHT_IMPL(STB_TEXTEDIT_STRING* obj, int idx)
{
  idx++;
  int len = obj->CurLenW;
...
static void
STB_TEXTEDIT_DELETECHARS(STB_TEXTEDIT_STRING* obj, int pos, int n)
{
...
static bool
STB_TEXTEDIT_INSERTCHARS(STB_TEXTEDIT_STRING* obj, int pos,
...
void
ImGuiTextEditState::OnKeyPressed(int key)
{
...
// nuisance.
void
ImGuiTextEditCallbackData::DeleteChars(int pos, int bytes_count)
{
...
void
ImGuiTextEditCallbackData::InsertChars(int pos, const char* new_text,
...
static bool
InputTextFilterCharacter(unsigned int* p_char, ImGuiInputTextFlags flags,
...
bool
ImGui::InputTextEx(const char* label, char* buf, int buf_size,
                   ...
                   ImGuiTextEditCallback callback, void* user_data)
{
    ...
    if (g.ActiveId != id) {
      ...
      edit_state.CurLenA =
        (int)(buf_end - buf); // We can't get the result from ImFormatString()
                              // above because it is not UTF-8 aware. Here we'll
                              // cut off malformed UTF-8.
  ...
  if (g.ActiveId == id || is_currently_scrolling) {
    ...
    draw_window->DrawList->AddText(
      ...
      GetColorU32(ImGuiCol_Text), buf_display, buf_display + edit_state.CurLenA,
    ...
    if (is_multiline)
      text_size =
        ImVec2(size.x, InputTextCalcTextLenAndLineCount(buf_display, &buf_end) *
...
bool
ImGui::InputScalarEx(const char* label, ImGuiDataType data_type, void* data_ptr,
                     ...
                     const char* scalar_format, ImGuiInputTextFlags extra_flags)
{
  ...
  if (step_ptr)
    PushItemWidth(ImMax(1.0f, CalcItemWidth() -
...
bool
ImGui::InputInt(const char* label, int* v, int step, int step_fast,
                ImGuiInputTextFlags extra_flags)
{
  ...
  return InputScalarEx(
    label, ImGuiDataType_Int, (void*)v, (void*)(step > 0.0f ? &step : NULL),
    (void*)(step_fast > 0.0f ? &step_fast : NULL), scalar_format, extra_flags);
}

bool
ImGui::InputFloatN(const char* label, float* v, int components,
                   int decimal_precision, ImGuiInputTextFlags extra_flags)
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
...
bool
ImGui::InputFloat2(const char* label, float v[2], int decimal_precision,
                   ImGuiInputTextFlags extra_flags)
{
...
bool
ImGui::InputFloat3(const char* label, float v[3], int decimal_precision,
                   ImGuiInputTextFlags extra_flags)
{
...
bool
ImGui::InputFloat4(const char* label, float v[4], int decimal_precision,
                   ImGuiInputTextFlags extra_flags)
{
...
bool
ImGui::InputIntN(const char* label, int* v, int components,
                 ImGuiInputTextFlags extra_flags)
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
...
bool
ImGui::InputInt2(const char* label, int v[2], ImGuiInputTextFlags extra_flags)
{
  return InputIntN(label, v, 2, extra_flags);
}

bool
ImGui::InputInt3(const char* label, int v[3], ImGuiInputTextFlags extra_flags)
{
  return InputIntN(label, v, 3, extra_flags);
}

bool
ImGui::InputInt4(const char* label, int v[4], ImGuiInputTextFlags extra_flags)
{
  return InputIntN(label, v, 4, extra_flags);
}

static bool
Items_ArrayGetter(void* data, int idx, const char** out_text)
{
...
static bool
Items_SingleStringGetter(void* data, int idx, const char** out_text)
{
...
bool
ImGui::Combo(const char* label, int* current_item,
                  ...
                  ImVec2 popup_size)
{
  ...
  const ImGuiWindowFlags flags =
    ImGuiWindowFlags_ComboBox | ((window->Flags & ImGuiWindowFlags_ShowBorders)
                                   ? ImGuiWindowFlags_ShowBorders
                                   : 0);
...
// PushID/PopID.
bool
ImGui::Selectable(const char* label, bool selected, ImGuiSelectableFlags flags,
...
bool
ImGui::Selectable(const char* label, bool* p_selected,
...
bool
ImGui::ListBoxHeader(const char* label, const ImVec2& size_arg)
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
    return false;
...
bool
ImGui::ListBoxHeader(const char* label, int items_count, int height_in_items)
{
  // Size default to hold ~7 items. Fractional number of items helps seeing that
  // we can scroll down/up without looking at scrollbar.
...
bool
ImGui::ListBox(const char* label, int* current_item, const char* const* items,
...
bool
ImGui::ListBox(const char* label, int* current_item,
               bool (*items_getter)(void*, int, const char**), void* data,
               int items_count, int height_in_items)
{
                ...
                bool enabled)
{
  ...
  if (window->DC.LayoutType == ImGuiLayoutType_Horizontal) {
    ...
    pressed =
      Selectable(label, false, flags | ImGuiSelectableFlags_DrawFillAvailWidth,
                 ImVec2(w, 0.0f));
...
bool
ImGui::MenuItem(const char* label, const char* shortcut, bool* p_selected,
                bool enabled)
{
...
ImGui::BeginMainMenuBar()
{
  ...
  SetNextWindowSize(
    ImVec2(g.IO.DisplaySize.x, g.FontBaseSize + g.Style.FramePadding.y * 2.0f));
  PushStyleVar(ImGuiStyleVar_WindowRounding, 0.0f);
...
void
ImGui::ColorTooltip(const char* text, const float* col,
...
static inline ImU32
ImAlphaBlendColor(ImU32 col_a, ImU32 col_b)
{
...
void
ImGui::RenderColorRectWithAlphaCheckerboard(ImVec2 p_min, ImVec2 p_max,
  ...
  if (((col & IM_COL32_A_MASK) >> IM_COL32_A_SHIFT) < 0xFF) {
    ...
    for (float y = p_min.y + grid_off.y; y < p_max.y; y += grid_step, yi++) {
      ...
      for (float x = p_min.x + grid_off.x + (yi & 1) * grid_step; x < p_max.x;
        ...
        if (y2 >= p_max.y) {
          if (x1 <= p_min.x)
            rounding_corners_flags_cell |= ImGuiCorner_BotLeft;
          if (x2 >= p_max.x)
...
ImGui::SetColorEditOptions(ImGuiColorEditFlags flags)
{
  ...
  IM_ASSERT(ImIsPowerOfTwo(
    (int)(flags &
          ImGuiColorEditFlags__InputsMask))); // Check only 1 option is selected
  IM_ASSERT(ImIsPowerOfTwo(
  ...
  if ((flags & ImGuiColorEditFlags_AlphaPreviewHalf) && col.w < 1.0f) {
    ...
    if (col_source.w < 1.0f)
      ...
      window->DrawList->AddRectFilled(bb_inner.Min, bb_inner.Max,
                                      GetColorU32(col_source), rounding, ~0);
  }
  if (window->Flags & ImGuiWindowFlags_ShowBorders)
...
static void
ColorPickerOptionsPopup(ImGuiColorEditFlags flags, float* ref_col)
{
...
static void
RenderArrow(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz,
...
static void
RenderArrowsForVerticalBar(ImDrawList* draw_list, ImVec2 pos, ImVec2 half_sz,
                           float bar_w)
{
...
bool
ImGui::ColorPicker4(const char* label, float col[4], ImGuiColorEditFlags flags,
                    const float* ref_col)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = GetCurrentWindow();
  ...
  if (alpha_bar) {
    ...
    RenderColorRectWithAlphaCheckerboard(
      bar1_bb.Min, bar1_bb.Max, IM_COL32(0, 0, 0, 0), bar1_bb.GetWidth() / 2.0f,
      ImVec2(0.0f, 0.0f));
    draw_list->AddRectFilledMultiColor(
...
ImGui::EndGroup()
{
  ...
  IM_ASSERT(
    !window->DC.GroupStack.empty()); // Mismatched BeginGroup()/EndGroup() calls
...
void
ImGui::SameLine(float pos_x, float spacing_w)
{
  ImGuiWindow* window = GetCurrentWindow();
  if (window->SkipItems)
    return;
...
static float
OffsetNormToPixels(ImGuiWindow* window, float offset_norm)
{
  return offset_norm * (window->DC.ColumnsMaxX - window->DC.ColumnsMinX);
}

static float
PixelsToOffsetNorm(ImGuiWindow* window, float offset)
{
...
static float
GetDraggedColumnOffset(int column_index)
{
...
void
ImGui::BeginColumns(const char* id, int columns_count, ImGuiColumnsFlags flags)
{
  ImGuiContext& g = *GImGui;
  ImGuiWindow* window = GetCurrentWindow();
  ...
  for (int column_index = 0; column_index < columns_count; column_index++) {
    ...
    float clip_x2 =
      ImFloor(0.5f + window->Pos.x + GetColumnOffset(column_index + 1) - 1.0f);
      ...
      if (!(window->DC.ColumnsFlags & ImGuiColumnsFlags_NoResize)) {
        ...
        if (held && g.ActiveIdIsJustActivated)
          g.ActiveIdClickOffset.x -=
            column_w; // Store from center of column line (we used a 8 wide rect
                      // for columns clicking). This is used by
...
void
ImGui::TreePushRawID(ImGuiID id)
{
  ImGuiWindow* window = GetCurrentWindow();
  Indent();
  window->DC.TreeDepth++;
...
static const char*
GetClipboardTextFn_DefaultImpl(void*)
{
...
static void
ImeSetInputScreenPosFn_DefaultImpl(int, int)
{
}
          ...
          while (clipper.Step())
            for (int prim = clipper.DisplayStart,
              ...
              for (int n = 0; n < 3; n++, vtx_i++) {
                ...
                buf_p +=
                  ImFormatString(buf_p, (int)(buf_end - buf_p),
